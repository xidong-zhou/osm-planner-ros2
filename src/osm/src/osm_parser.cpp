...
namespace osm_planner {
   ...
   void Parser::initialize(rclcpp::Node::SharedPtr& node){
	...
	position_marker_pub = n->create_publisher<visualization_msgs::msg::Marker>("position_marker",5);
	target_marker_pub = n->create_publisher<visualization_msgs::msg::Marker>("target_marker",1);
	path_pub = n->create_publisher<nav_msgs::msg::Path>("route_network",10);
	refused_path_pub = n->create_publisher<nav_msgs::msg::Path>("refused_path",10);
    ...
    void Parser::parse(bool onlyFirstElement) {
        ...
        if (loadOkay) {
            RCLCPP_INFO(n->get_logger(),"OSM planner: loaded map: %s", xml.c_str());
        } else {
            RCLCPP_ERROR(n->get_logger(),"OSM planner: Failed to load file %s", xml.c_str());
    ...
    void Parser::publishPoint(geometry_msgs::msg::Point point, int marker_type, double radius, geometry_msgs::msg::Quaternion orientation) {
    ...
    void Parser::publishPoint(double latitude, double longitude, int marker_type, double radius, geometry_msgs::msg::Quaternion orientation) {
    ...
    void Parser::publishPoint(int pointID, int marker_type, double radius,geometry_msgs::msg::Quaternion orientation) {
    ...
    void Parser::publishPoint(const OSM_NODE &node, int marker_type, double radius,geometry_msgs::msg::Quaternion orientation) {
...
//publishing defined path
    void Parser::publishRefusedPath(std::vector<int> nodesInPath) {
        ...
        nav_msgs::msg::Path refused_path;
        ...
        refused_path.header.stamp = clock.now();
...
//getter for dijkstra algorithm - getting only pointer for spare memory
    ...
    //TODO std::shared_ptr
    nav_msgs::msg::Path Parser::getPath(std::vector<int> nodesInPath) {

        //msgs for shortest path
        nav_msgs::msg::Path sh_path;

        sh_path.poses.clear();
        sh_path.header.frame_id = map_frame;
        sh_path.header.stamp = clock.now();
...
//private functions
   ...
   void Parser::createMarkers() {
        ...
        position_marker.header.stamp =  clock.now();
        ...
        position_marker.type = visualization_msgs::msg::Marker::CYLINDER;

        position_marker.scale.z = 0.05;

        position_marker.lifetime = rclcpp::Duration(100);
...
//Finding nodes by OSM ID
   Parser::OSM_NODE_WITH_ID Parser::getNodeByOsmId(std::vector<OSM_NODE_WITH_ID> nodes, int id) {
        ...
        RCLCPP_ERROR(n->get_logger(),"OSM planner: nenaslo ziadnu nodu - toto by sa nemalo stat");
        return nodes[0];
   }
            ...
            for (int j = 0; j < ways[i].nodesId.size() - 1; j++) {
              ...
              //  ROS_INFO("dist %f", distance);
                ...
                if (networkArray[ways[i].nodesId[j]][ways[i].nodesId[j + 1]] != 0)
                    RCLCPP_ERROR(n->get_logger(),"OSM planner: pozicia [%d, %d] je obsadena - toto by nemalo nastat", ways[i].nodesId[j],
                              ways[i].nodesId[j + 1]);
